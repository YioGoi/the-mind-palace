...
export default function NiceScreen() {
  ...
  useEffect(() => {
    // default all sections to collapsed when sections change
    setCollapsedSections((prev) => {
      const next = { ...prev }
  ...
  const displaySections = React.useMemo(
    () =>
      sections.map((s) => ({
        ...s,
        // keep a single placeholder item when collapsed so SectionList still renders the header
  ...
  // Map of section id -> real note count (uses original sections data, not display placeholder)
    ...
    <SafeAreaView style={styles.container}>
      {loading ? (
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={Palette.colorAccent} />
        </View>
      ) : (
        <>
          <View style={styles.bannerWrapper}>
            <Image source={require('@/assets/images/nice-banner.png')} style={styles.banner} contentFit="contain" accessibilityLabel="Nice banner" />
          </View>
                  ...
                  <Text style={styles.sectionTitle}>
                    ...
                    {hasPending ? (
                      <View style={{ marginLeft: 8, transform: [{ translateY: 2 }] }}>
                        <ActivityIndicator size="small" color={Palette.colorAccent} />
                      </View>
                    ) : sectionCounts[section.id] ? (
                      <Text style={styles.sectionCount}> ({sectionCounts[section.id]})</Text>
                    ) : null}
                  </Text>
                  <IconSymbol name="chevron.right" size={22} color={Palette.colorAccent} style={{ transform: [{ rotate: collapsedSections[section.id] ? '0deg' : '90deg' }] }} />
                </TouchableOpacity>
              )
            }}
...
const styles = StyleSheet.create({
  ...
  sectionList: { flex: 1 },
  sectionHeader: { padding: 12, backgroundColor: Palette.colorAccentSoft, borderBottomWidth: 1, borderBottomColor: Palette.colorBorder, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  sectionTitle: { fontSize: 16, fontWeight: '700', color: Palette.colorAccent },
  sectionCount: { fontSize: 14, color: Palette.colorAccent, opacity: 0.8 },
  sectionIndicator: { fontSize: 16, color: Palette.colorAccent },
})